...
# 创投人物
# 爬虫地址https://www.itjuzi.com/person
url = config.get("spiderAPI", "personsSpider")
...
# 爬虫主体
def getInfoByPageNum(pageNum, headers=headers0, personType='1'):
    payload = "{\"per_page\":20,\"page\":%s,\"type\":%s,\"scope\":\"\",\"sub_scope\":\"\",\"round\":\"\",\"prov\":\"\",\"city\":\"\",\"status\":\"\",\"sort\":\"\",\"selected\":\"\"}" % (pageNum, personType)
    try:
        # headers = getHeaders()
...
def insertToMongo(rows):
    if rows:
        for row in rows:
            user_id = row['id']
            # 获取详细信息
            url = "https://www.itjuzi.com/api/persons/%s" % user_id
            response = requests.request("GET", url).text
            response = json.loads(response)

            row = dict(row, **response['data'])
...
if __name__ == '__main__':
    for personType in range(1, 4):
        pageNumNow = 1
        totalPage, pageNumNow, rows = getInfoByPageNum(pageNum=pageNumNow, personType=personType)
        print(personType, '一共有：%s页' % totalPage)
        print('正在爬取第%s/%s页' % (pageNumNow, totalPage))
        while pageNumNow <= totalPage:
            insertToMongo(rows=rows)
            try:
                totalPage, pageNumNow, rows = getInfoByPageNum(pageNum=pageNumNow + 1, personType=personType)
